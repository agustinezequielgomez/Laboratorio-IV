...
class EstrellaDeCineController
{
  ...
  public static function Create(Request $request, Response $response, $args)
  {
    ...
    $estrellaDeCine = new EstrellaDeCine;
    $estrellaDeCine->nombre = $data["nombre"];
    $estrellaDeCine->apellido = $data["apellido"];
    $estrellaDeCine->nacionalidad = $data["nacionalidad"];
    $estrellaDeCine->fechaDeNacimiento = $data["fechaDeNacimiento"];
    $estrellaDeCine->save();
  ...
  {
    $data = $request->getParsedBody();
    $venta = EstrellaDeCine::find($args["id"]);
    if(!$venta)
    {
  ...
  public static function Delete(Request $request, Response $response, $args)
  {
    ...
    $producto = EstrellaDeCine::find($args["id"]);
    if(!$producto)
    {
      return $response->withJson("producto inexistente", 200);
    }
